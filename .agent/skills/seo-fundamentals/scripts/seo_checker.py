...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if this is a layout/template file or Next.js metadata provider
    is_layout = False # Next.js App Router sets this differently 
    has_next_metadata = 'export const metadata' in content or 'export const generateMetadata' in content
    
    # 1. Title tag
    has_title = '<title' in content.lower() or 'title=' in content or 'Head>' in content or 'title:' in content or 'title ' in content
    if not has_title and is_layout and not has_next_metadata:
    ...
    if not has_description and is_layout and not has_next_metadata:
        issues.append("Missing meta description")
    
    # 3. Open Graph tags
    has_og = 'og:' in content or 'property="og:' in content.lower() or 'openGraph:' in content
    if not has_og and is_layout and not has_next_metadata:
